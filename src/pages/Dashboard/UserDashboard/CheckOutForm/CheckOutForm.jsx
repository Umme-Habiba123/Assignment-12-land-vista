...
const CheckoutForm = ({ offer }) => {
  ...
  // ✅ Step 0: Show paid message if already bought
  const isBought = offer?.status === "bought";

  useEffect(() => {
    if (!isBought && usdAmount > 0) {
      axiosSecure
  ...
  }, [usdAmount, axiosSecure, isBought]);
    ...
    if (paymentIntent.status === "succeeded") {
      ...
      try {
        ...
        if (paymentRes.data.insertedId) {
          ...
          Swal.fire("✅ Payment Successful!", `Transaction ID: ${paymentIntent.id}`, "success").then(() => {
